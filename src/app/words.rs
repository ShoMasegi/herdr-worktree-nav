//! What the plugin says when there is no picker on screen to say it.
//!
//! A removal outlives the picker that started it (`docs/adr/0014-removing-outlives-the-picker.md`),
//! so `herdr-worktree-nav remove` runs with no terminal at all and the toast it shows is
//! composed here, next to the process that sends it. Everything the picker itself says is
//! [`ui::words`](crate::ui::words); `domain` composes neither.
//!
//! The prompt-line half is here too, because it is the same sentence: the toast is the
//! report that always happens, and the prompt line is the extra the picker adds when it is
//! still up to read the answer. Two readers, one set of words.
...
mod tests {
    use super::*;
    use crate::port::NotificationSound;
