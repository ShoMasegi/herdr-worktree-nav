...
`←` `→` はリポジトリの中で最初に行き先となる行——最初の pane、無ければ最初の「何も動いていない checkout」——に着きます。**1 押しは必ず 1 リポジトリ**で、そのリポジトリの途中にいても同じです。`←` は今いるリポジトリの先頭に戻るのではなく、そこから出ます。端では巻き戻り、リポジトリに属さない pane 群も 1 セクションとして数えます。矢印は文字ではないので、検索ボックスに入力中でも効きます。
