...
報告するのは子プロセスで、ピッカーはそれを飾るだけです。パイプに書いた 1 行が読まれたかどうかは、ループにも子にも分かりません——利用者は Branches ビューにいるかもしれず、既に閉じているかもしれない——ので、通知は無条件に出し、成功時にピッカーは何も足しません。一覧を読み直せなかったときだけは別で、消えるはずだった行が画面に残っているため、読み直せなかったことを言います。`setsid` は本質的な部分です。herdr は閉じた pane のプロセスグループを殺すためです（[ADR 0014](../adr/0014-removing-outlives-the-picker.md)）。
...
この表について、代償を払って得た規則が 2 つあります。

**失敗したときの値が、成功したときにもありうる値である関数には、成功を見るテストが 1 つ要ります。**
空のリスト、`None`、空文字列。空の答えしか見たことのないテストは、それを「常に空を返す関数」と
区別できません。`GhPort::pull_requests` は `gh` が無いとき、断られたとき、出力が読めなかったとき、
そして単にそのリポジトリに pull request が無いときに、いずれも空のリストを返します。「常に空」は、
あるテストがそれに 1 件名乗らせるまで、スイート全体を素通りしました。

**変異は `cargo test --all-targets` で測ります。それより狭いもので測ってはいけません。**
1 つのターゲットだけを走らせて生き残った変異は発見ではなく、計器を別の場所に向けているだけです。
`--test gh_cli` だけを走らせたために、単体テストが 1 つ押さえている行について「この exit チェックには
テストが無い」という文が pull request の本文に載りました。
