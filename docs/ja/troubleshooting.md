...
そこに入る理由はもう 1 つあります。pane がどこにあるかを git に訊けなかった場合です。**セッション全体**がその見出しの下にあるときは、まずこちらを疑ってください。herdr がプラグインを起動した `PATH` に `git` が無ければ、全 pane が同時に失敗するからです。プロンプト行は git 自身の言葉でそれを言い、リポジトリ単位の文より先に出します。`dump` は該当する pane の下に理由を印字します。
