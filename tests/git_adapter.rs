...
fn a_checkout_that_has_gone_is_an_answer_rather_than_a_git_that_could_not_be_run() {
    ...
    // here as "git could not be asked", which is the sentence `docs/{en,ja}/troubleshooting`
    // sends the reader to check `PATH` over, and this is not that — so it has to be the
    // ordinary answer.
    //
    ...
    // path gives — and this row would name the wrong cause. That is what this pins.
