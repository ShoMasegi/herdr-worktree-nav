...
各 checkout は自分が今どういう状態かも名前の脇で言います。`↑2↓1` は upstream からどれだけずれているか、`gone` は追跡している ref を git が見つけられないこと、`✱` は作業ツリーが誰もコミットしていない作業を抱えていること——`git worktree remove` が断る理由です——そして `?` は git がその作業ツリーを読めなかったことです。

`Enter` でそこへ移動します。space をまたいでも tab をまたいでも、目的の pane に直接飛びます。`p` で pane が 1 つも無い worktree の表示と非表示を切り替えます。表示後、その行で `Enter` を押すと開きます。これらの worktree は既定で表示されますが、プラグインの設定で初期値を変更できます。`←`/`→` で前／次のリポジトリの先頭に飛べます。`b`/`w`/`i`/`d` でエージェントの状態を 1 つに絞り込み、`a` で解除できます（navigator と同じキーです）。
